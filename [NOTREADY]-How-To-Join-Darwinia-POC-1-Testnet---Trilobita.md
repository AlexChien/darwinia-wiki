...
再回到General界面，打开右上角的custom endpoint开关，然后在remote node/endpoint to connect to 填入：

    wss://trilobita.darwinia.network/

输入完成后点击下方最右侧蓝色button Save&Reload。这时页面就会自动刷新到可用界面。选择左侧的Explorer可看到如下：
...
点击左侧toolbar中的Accounts可以看到如下界面，点击右上角Add account：
...
生成之后点击Save， 之后会弹出一个对话询问你是否要保存账户信息到本地，可以继续点击Create and backup account.



这时就可以在Accounts界面看到我们最新生成的账户地址。
...
关于ring的转账，在Transfer页面完成。

3-1 获得KTON，用于Staking

获得KTON的唯一途径，就是存RING，在extrinsics页面中，选择操作账户（这里是tribolita_stash），选择kton -> deposit，依次填入需要存入的RING的数量和存入的月数（1-36)， 之后点击Submit Transaction：



右上角会出现该次操作的事件，不过因为波卡官方和substrate之间存在版本不匹配的问题，所以事件确认目前无法使用，需要手动确认状态。等达尔文钱包上线之后，操作会方便很多。

这时，我们需要去手动确认链上状态，在deposit之后，系统会锁定用户的这部分RING，并且增发出KTON给用户。我们在Chain state界面中选择 ktonBalances -> freeBalance，选择需要查看的账户，点击左右侧的蓝色+号，即会出现该账户的KTON余额：
...
点击确认之后，继续去Chainstate查看状态。Chain state -> staking -> bonded，选择上一步的操作账户（tribolita_stash），如果成功的话就会出现刚才填入的controller账户，失败的话就会显示<empty>.
...
重复之前的账户生成， 在Accounts 生成sessionkey。需要注意的是，在点击Add account之后，在接下来跳出的界面中选择ed25519(之前是sr25519)，seed一栏建议选用 Raw seed（建议把seed保存下来）：
...
在extrinsics -> session -> setKeys界面，输入如下参数（注意！：这里的操作人是tribolita，即之前bond时填入的controller）：
...
这时去Chain state -> session -> nextKeyFor查看，参数输入tribolita, 如果结果不是<empty>就说明绑定成功：
...
先打开Staking页面，查看当前的验证人情况：





打开Extrinsics -> staking -> validate, 此时操作账户依然为controller，
...
操作之后进入Staking界面，可以在 next up中看到我们的账户了，说明此时已进入验证人候选名单中，只需要等待下一轮换届即可。
...
在#250块可以很清晰地看到，之前只是监听同步全网块高，在#250之后日志开始验证。
...
下面我们新建一个账号MAG，存入一些RING生成KTON之后，可以看到MAG的RING和KTON数量如下（此时MAG并没有做Staking）：



经过一段时间之后，我们可以使用Extrinsics -> kton -> claimReward 来领取未锁定KTON的系统奖励，之后再回到Chain state中查看MAG当前的RING余额：
